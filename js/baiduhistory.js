...
function tohist() {
  if (document.getElementById("history-container")) {
    ...
    function history_get_data() {
      ...
      return [
        "https://fastly.jsdelivr.net/npm/butterfly-card-history/json/" + getMonth + ".json",
        getMonthDate
      ]
    }
    var history_data = history_get_data()
    void fetch(history_data[0])
      .then((data) => data.json())
      .then((data) => {
        ...
        for (var item of data[history_data[1]]) {
          html_item +=
            '<div class="swiper-slide history_slide"><span class="history_slide_time">A.D.' +
            item.year +
            "</span>" +
            '<span class="history_slide_link">' +
            item.title +
            "</span></div>"
        }
        ...
        if (typeof Swiper !== "undefined") {
          swiper_history = new Swiper(".history_swiper-container", {
          ...
          console.warn("Swiper library is not loaded. History feature will not work.")
        }
      })
  }
}
