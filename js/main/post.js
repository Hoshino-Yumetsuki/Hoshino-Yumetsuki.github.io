...
window.AcrypleMainPost = (() => {
  const addHighlightTool = () => {
    ...
    const isHighlightShrink =
      typeof GLOBAL_CONFIG_SITE !== "undefined" ? GLOBAL_CONFIG_SITE.isHighlightShrink : undefined
    ...
    switchBtn.addEventListener("click", function () {
      ...
      document.querySelectorAll("#post-comment > .comment-wrap > div").forEach((item) => {
        if (btf.isHidden(item)) {
          item.classList.remove("comment-wrap-hide")
          item.classList.add("comment-wrap-switch")
        } else {
          item.classList.remove("comment-wrap-switch")
          item.classList.add("comment-wrap-hide")
        }
      })
