  ...
  var aCalendar = function (language, options, object) {
    ...
    function parse() {
      ...
      if (
        ...
        ((current.prev === null && current.next !== null && current.next.getTime() > time) ||
          (current.next === null && current.prev !== null && current.prev.getTime() < time))
      ) {
        return false
      }
    ...
    function draw() {
      ...
      if (current.posts.length === 0) {
        ...
        cTitleLink = $("<a/>")
          .attr("onclick", `pjax.loadUrl('${simpleDateFormat(curDate, settings.titleLinkFormat)}')`)
      ...
      if (current.prev) {
        cPrevPosts
          .html(settings.footArrows.previous + settings.months[current.prev.getMonth()])
          .addClass("cal-foot")
      ...
      if (current.next) {
        cNextPosts
          .html(settings.months[current.next.getMonth()] + settings.footArrows.next)
          .addClass("cal-foot")
        ...
        for (var j = 0; j < 7; j++) {
          ...
          if (i * 7 + j < dWeekDayOfMonthStart) {
            ...
            if (count.num !== 0) {
              ...
              var cLink = $("<a>")
                .attr("onclick", `pjax.loadUrl('${current.posts[index].link}')`)
